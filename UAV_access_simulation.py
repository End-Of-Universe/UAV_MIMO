...
class UAV_environment:
    ...
    def run_simulation(self):
        self.ue_num = config["Access_Dataset_Config"]["ue_num"]
        self.times_pilot = 1
        bs_search_r = 3
        ue_search_r = 3
        self.__initialize_environment()
        print("UAV accessor's train dataset is generating...")

        # prob_bs = np.zeros((self.ue_num, bs_search_r * 2 + 1, bs_search_r * 2 + 1), dtype = np.int16)
        # prob_ue = np.zeros((self.ue_num, ue_search_r * 2 + 1, ue_search_r * 2 + 1), dtype = np.int16)

        label_bs = np.zeros(self.ue_num, dtype = np.int64)
        label_ue = np.zeros(self.ue_num, dtype = np.int64)
        ...
        for user in range(self.ue_num):
            delta_pos0 = delta_pos[user]
            ue_rot0 = ue_rot[user]
            h0 = h_t[user, :, :]
            theta_zod, phi_aod, theta_zoa, phi_aoa = utils.calculate_los_angle(delta_pos0, ue_rot0)
            z_center_bs, y_center_bs = utils.calculate_upa_index(theta_zod, phi_aod, self.bs_z_antenna_num, self.bs_y_antenna_num)
            z_center_ue, y_center_ue = utils.calculate_upa_index(theta_zoa, phi_aoa, self.ue_z_antenna_num, self.ue_y_antenna_num)
            _, _, (z_bs_s, y_bs_s, z_ue_s, y_ue_s), _ = initial_access_3(h0, delta_pos0, ue_rot0, codebook, bs_search_r, ue_search_r)

            # prob_bs[user, z_bs_s, y_bs_s] = 1
            # prob_ue[user, z_ue_s, y_ue_s] = 1

            label_bs[user] = z_bs_s + y_bs_s * (2 * bs_search_r + 1)
            label_ue[user] = z_ue_s + y_ue_s * (2 * ue_search_r + 1)
            
            # # _, _, rsrp1 = initial_access_3(h0, delta_pos0, ue_rot0, codebook, bs_search_r, ue_search_r)
            # print("Access --- %d/%d ---" % (user + 1, self.ue_num))
            # print("way1: dz_bs - %d, dy_bs - %d, dz_ue - %d, dy_ue - %d, rsrp: %.3f" % (z_bs_opt, y_bs_opt, z_ue_opt, y_ue_opt, max_psd))
            # _, _, (dz_bs, dy_bs, dz_ue, dy_ue), rsrp1 = initial_access_3(h0, delta_pos0, ue_rot0, codebook, bs_search_r, ue_search_r)
            # print("way2: dz_bs - %d, dy_bs - %d, dz_ue - %d, dy_ue - %d, rsrp: %.3f" % (dz_bs, dy_bs, dz_ue, dy_ue, rsrp1))

        dataset_pth = config["Access_Dataset_Config"]["dataset_pth"] % (config["Access_Dataset_Config"]["ue_num"])
        access_dataset_h5 = h5py.File(dataset_pth, "w")
        random_index = np.arange(self.ue_num, dtype = np.int16)
        np.random.shuffle(random_index)
        access_dataset_h5.create_dataset("ue_num", data = self.ue_num, dtype = np.int16)
        access_dataset_h5.create_dataset("delta_pos", data = delta_pos, dtype = np.float32)
        access_dataset_h5.create_dataset("ue_rot", data = ue_rot, dtype = np.float32)
        # access_dataset_h5.create_dataset("prob_bs", data = prob_bs)
        # access_dataset_h5.create_dataset("prob_ue", data = prob_ue)
        access_dataset_h5.create_dataset("label_bs", data = label_bs, dtype = np.int64)
        access_dataset_h5.create_dataset("label_ue", data = label_ue, dtype = np.int64)
        access_dataset_h5.create_dataset("random_index", data = random_index, dtype = np.int16)
        access_dataset_h5.close()
        print("Access dataset has been saven!")
