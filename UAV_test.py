...
class UAV_environment:
    ...
    def test(self, 
        ...
        model.load(model_dict[model_type]["model_pth"])
        ...
        for self.t in range(self.times_pilot):
            ...
            for user in range(self.ue_num):
                ...
                if snr_baseline < threshold or count_baseline[user] >= track_period_baseline:
...
def plot_track_curve(user = 0):
    ...
    plt.legend()
    plt.title("SNR Curve of Baseline Tracking")
    ...
    plt.legend()
    plt.title("SNR Curve of Data-driven Tracking")
...
def plot_expense_bar():
    ...
    plt.xlabel("track period (number of CSI period)")
    plt.ylabel("sum of expense")
    plt.grid(True, linestyle='--')
    plt.title("Track Expense Distribution of Baseline Tracking")
    ...
    plt.ylabel("sum of expense")
    plt.grid(True, linestyle='--')
    plt.title("Track Expense Distribution of Data-driven Tracking")
